...
public class Sort {
     ...
     * @throws ClassCastException If the vector contains objects that are not
     *                            <code>Ordered</code>.
     */
    ...
    public static void QuickSort(Vector v, int lo0, int hi0) throws ClassCastException {
        ...
        if (hi0 > lo0) { // arbitrarily establish partition element as the midpoint of the
            // vector
    ...
    public static void QuickSort(Sortable sortable, int lo0, int hi0) {
        ...
        if (hi0 > lo0) { // arbitrarily establish partition element as the midpoint of the
            ...
            while (lo <= hi) {
                ...
                // if the indexes have not crossed, swap
                if (lo <= hi) sortable.swap(lo++, hi--);
            }

            // if the right index has not reached the left side of array
            // must now sort the left partition
            if (lo0 < hi) QuickSort(sortable, lo0, hi);

            // if the left index has not reached the right side of array
            // must now sort the right partition
            if (lo < hi0) QuickSort(sortable, lo, hi0);
        }
    }
     ...
     * @throws ClassCastException If the keys of the hashtable are not <code>Ordered</code>.
     */
    public static Object[] QuickSort(Hashtable h) throws ClassCastException {
        ...
        for (int i = 0; i < ret.length; i++) {
            ret[i] = e.nextElement();
            if (are_strings && !(ret[i] instanceof String)) are_strings = false;
        }
