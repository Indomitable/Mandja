...
public class CssSelectorNodeFilter implements NodeFilter {
    ...
    private boolean nextToken() {
        if (m != null && m.find()) for (int i = 1; i < m.groupCount(); i++)
            if (null != m.group(i)) {
                tokentype = i;
                token = m.group(i);
                return true;
                        ...
                        switch (token.charAt(0)) {
                            case '.':
                                ...
                                if (ret == null)
                                    ...
                                    ret = new AndFilter(ret, new HasAttributeFilter("class",
                                            unescape(token)));
                                break;
                            case '#':
                                nextToken();
                                ...
                                if (ret == null)
                                    ...
                                    ret = new AndFilter(ret, new HasAttributeFilter("id",
                                            unescape(token)));
                                break;
                            case ':':
                                nextToken();
    ...
    private NodeFilter parseAttributeExp() {
        ...
        if (tokentype == NAME) {
            ...
            if ("]".equals(token))
                ...
                else if (tokentype == NAME) val = unescape(token);
                if ("~=".equals(rel) && val != null)
                    ret = new AttribMatchFilter(unescape(attrib), "\\b"
                            + val.replaceAll("([^a-zA-Z0-9])", "\\\\$1") + "\\b");
                else if ("=".equals(rel) && val != null) ret = new HasAttributeFilter(attrib, val);
            }
        }
        if (ret == null)
    ...
    public static String unescape(String escaped) {
        ...
        while (m.find()) {
            ...
            else if (m.group(2) != null) m.appendReplacement(result, m.group(2));
        }
        m.appendTail(result);
        ...
        public boolean accept(Node n) {
            if (n.getParent() != null) {
                ...
                for (int i = 0; i < l.size(); i++)
                    if (l.elementAt(i) == n && i > 0) return (sibtest.accept(l.elementAt(i - 1)));
            }
            return false;
        }
    }
    ...
    private static class AttribMatchFilter implements NodeFilter {
        ...
        public boolean accept(Node node) {
            if (node instanceof Tag && ((Tag) node).getAttribute(attrib) != null)
                if (rel != null && !rel.matcher(((Tag) node).getAttribute(attrib)).find())
                    return false;
                else
                    return true;
            else
