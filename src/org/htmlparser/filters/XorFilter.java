...
public class XorFilter implements NodeFilter {
    ...
    public void setPredicates(NodeFilter[] predicates) {
        if (null == predicates) predicates = new NodeFilter[0];
     ...
     * @return <code>true</code> if an odd number of the predicate filters find
     *         the node is acceptable, <code>false</code> otherwise.
     */
    public boolean accept(Node node) {
        ...
        for (int i = 0; i < mPredicates.length; i++)
            if (mPredicates[i].accept(node)) ++countTrue;
